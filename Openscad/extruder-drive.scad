	...
	difference(){
		union(){
			if(!frame_clamp)
				...
				translate([1,5-0.5,7]) cube([motor_hole_pitch+11,31+17,14],center=true);
			translate([17,24,3]) rotate([0,-90,0]){
				cylinder(r=3,h=40);
				translate([-3,0,0]) cube([3,3,40]);
			}
				//boss for carriage mount hole
				rotate([90,0,0]) translate([20,7,(31-7)/2]) rotate([0,0,22.5]) cylinder(r=14*da8,h=7.5,$fn=8);
      ...
      translate([-9,0,2.5]) cube([24,11,5],center=true);
		...
		translate([filament_offset_x,0,7.5]) rotate([90,0,0]){
			...
			translate([0,0,11.1+1]) {
                ...
                translate([0,6,1.65]) cube([8.5,12,3.3], center=true);
